...
connect(Service, Req, Env) ->
    ...
    case hstub_client:connect(ranch_tcp,
                              ip_to_tuple(IpAddress),
                              Port,
                              100,
                              Client) of
        {ok, Client2} ->
            proxy(Req, #state{backend_client=Client2,
                              env=Env});
        {error, _} ->
            {error, 503, Req}
    end.
...
relay_full_body(Status, Headers, Req, State = #state{backend_client=Client}) ->
    case hstub_client:response_body(Client) of
        {ok, Body, _Client2} ->
            respond(Status, Headers, Body, Req); % @todo should the connection not be closed here?
        {error, _} ->
            backend_close(State),
            respond(503, [], [], Req)
    end.
...
respond(Status, Headers, Body, Req) ->
    {ok, Req1} = cowboy:reply(Status, Headers, Body, Req),
    {halt, Req1}.
